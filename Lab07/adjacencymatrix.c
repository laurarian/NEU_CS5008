...
int nodeInDegree(int* g, int node){
    ...
    for(int i = 0; i < ROWS; i++){
        // Check if there is a connection from node i to the specified node.
...
int nodeOutDegree(int* g, int node){
    ...
    for(int j = 0; j < COLUMNS; j++){
        // Check if there is a connection from the specified node to node i.
...
int isConnected(int* g, int node1, int node2){
    return g[node1 * COLUMNS + node2] == 1; // Returns 1 if nodes are connected, otherwise 0
}
