...
int unitTest6(int status){
    ...
    // Checks if the first item is correctly peeked without removing it
...
int unitTest7(int status){
    ...
    // Checks if the last item is correctly returned without removing it
...
// Test the queue_equal function
int unitTest8(int status){
    int passed = 0;
    queue_t* testq1 = create_queue(10);
    queue_t* testq2 = create_queue(10);

    queue_enqueue(testq1, 1);
    queue_enqueue(testq1, 2);

    queue_enqueue(testq2, 1);
    queue_enqueue(testq2, 2);

    if (queue_equals(testq1, testq2)) {
        passed = 1;
    }

    // Modify testq2 and test again
    queue_enqueue(testq2, 3);
    // Checks if two identical queues are considered equal
    if (queue_equals(testq1, testq2)) {
        passed = 0; // This should not be equal anymore
    }
