...
// Test stack_peek function
int unitTest6(int status){
    int passed = 0;
    
    stack_t* test_s = create_stack(MAX_DEPTH);
    stack_enqueue(test_s,10);
    stack_enqueue(test_s,20);
    
    int top_item = stack_peek(test_s);
    if(20 == top_item && 2 == stack_size(test_s)){ // Check top item and size should remain the same
        passed = 1;
    }
...
//Test stack_equals function
int unitTest7(int status){
    int passed = 0;
    
    stack_t* stack1 = create_stack(MAX_DEPTH);
    stack_t* stack2 = create_stack(MAX_DEPTH);
    
    // Enqueue the same items in both stacks
    stack_enqueue(stack1,1);
    stack_enqueue(stack2,1);
    stack_enqueue(stack1,2);
    stack_enqueue(stack2,2);

    if(stack_equals(stack1, stack2) == 1){ 
        passed = 1;
    } 
    free_stack(stack1);
    free_stack(stack2);
