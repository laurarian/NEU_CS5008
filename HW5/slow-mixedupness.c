...
// No additional helper functions needed for this brute-force implementation.
...
int mixedupness(int *array, unsigned int size)
{
    ...
    for (i = 0; i < size - 1; i++)
    {
        // For each element, compare it with every other element that comes after it.
        for (j = i + 1; j < size; j++)
        {
            // If an element later in the array is smaller, it means the pair is in reverse order.
            if (array[i] > array[j])
            {
                score++;
            }
